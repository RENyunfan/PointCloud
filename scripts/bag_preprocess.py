...
filepath='/media/harddrive-bay0/data_processing/data/yiyang_fullRes_10hz_large/yiyang_fullRes_10hz_large.bag'
...
def bag2image(topic_name,output_dir,num):
	with tqdm(total=num) as pbar:
		...
		count=0
		timestamp=[]
		for _, _, t in bag.read_messages(topics=topic_name):
...
for cam_name in camera_list:
	...
	if not os.path.isdir(cam_folder):
		os.mkdir(cam_folder)
	topic_name= '/camera_array/{}/image_raw'.format(cam_name)
